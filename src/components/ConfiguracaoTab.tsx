...
export default function ConfiguracaoTab({
  ...
  const [novoProfessor, setNovoProfessor] = useState({ nome: '', telefone: '' });
  ...
  const adicionarProfessor = () => {
    ...
    const professor: Professor = {
      ...
      telefone: novoProfessor.telefone.trim() || undefined,
      disciplinas: [],
      turmas: [],
      atribuicoes: [],
      aulasSemanais: 0,
    ...
    setNovoProfessor({ nome: '', telefone: '' });
    setProfessoresAbertos(prev => new Set([...prev, professor.id]));
  };

  const atualizarTelefoneProfessor = (id: string, telefone: string) => {
    setProfessores(professores.map(p =>
      p.id === id ? { ...p, telefone: telefone.trim() || undefined } : p
    ));
  };
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="flex gap-3">
              ...
              <input
                ...
                className="w-52 px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-400 focus:border-green-400 outline-none"
              />
              <button
                    ...
                    {isAberto && (
                      <div className={cn('border-t', cor.border)}>
                        ...
                        <div className="px-4 py-3 bg-green-50 border-t border-green-100 flex items-center gap-3">
                          ...
                          <label className="text-sm font-semibold text-green-800 whitespace-nowrap">WhatsApp:</label>
                          <input
                            type="text"
                            value={prof.telefone || ''}
                            onChange={(e) => atualizarTelefoneProfessor(prof.id, e.target.value)}
                            placeholder="Ex: 11999999999 (com DDD, sem espaços)"
                            className="flex-1 px-3 py-1.5 border border-green-300 rounded-lg text-sm focus:ring-2 focus:ring-green-400 outline-none bg-white"
                          />
                          {prof.telefone && (
                            <a
